...
module.exports = function repreprep(root) {
  ...
  var msgs = [
      'Welcome to replpad!'
    , 'Specifications:'
    , format('\tRunning node v%s (v8 %s, uv %s)', v.node, v.v8, v.uv)
    , format('\ton %s with %s cpus, platform: %s', specs.host, specs.cpus, specs.platform)
    ];

  if (!root) { 
    log.info('Watching no files since no path was specified.');
    msgs.forEach(function (msg) { log.info(msg); });
    return createRepl(stdin);
  }
