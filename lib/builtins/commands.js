...
module.exports = function commands(repl) {
  ...
  repl.defineCommand('pack', {
    ...
    action: function(){
      var packFile = path.join(process.cwd(), 'package.json')
      ...
      if(exists){
        packContents = require(packFile);
        deps = xtend(packContents.dependencies, packContents.devDependencies);
        Object.keys(deps)
          .forEach(function(dep){
            try {
              repl.context[dep] = require(dep);
            } catch(e) {
              log.error(format('Unable to load %s', dep));
            }
          });
      } else {
